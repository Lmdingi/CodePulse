...
namespace CodePulse.API.Repositories.Implementation
{
    public class BlogPostRepository : IBlogPostRepository
    {
        ...
        {
            return await dbContext.BlogPosts.Include(x => x.Categories).FirstAsync(x => x.Id == id);
        }

        public async Task<BlogPost?> GetUrlHandleAsync(string urlHandle)
        {
            return await dbContext.BlogPosts.Include(x => x.Categories).FirstAsync(x => x.UrlHandle == urlHandle);
        }
